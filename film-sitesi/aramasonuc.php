...
if (isset($_GET['s'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM filmler WHERE film_adi LIKE ? OR aciklama LIKE ? OR fragman_link LIKE ? OR izleme_link LIKE ?");
    ...
    <?php if (isset($result) && $result->num_rows > 0): ?>
        <div class="row">
            <?php while ($row = $result->fetch_assoc()): ?>
                <div class="col-md-4">
                    <div class="card mb-4">
                        <img src="uploads/<?php echo $row['kapak_foto']; ?>" class="card-img-top" alt="<?php echo $row['film_adi']; ?>">
                        <div class="card-body">
                            <h5 class="card-title"><?php echo $row['film_adi']; ?></h5>
                            <p class="card-text"><?php echo $row['aciklama']; ?></p>
                            <a href="filmler/tumfilmler/<?php echo strtolower(str_replace(' ', '-', $row['film_adi'])); ?>.php" class="btn btn-primary">Detaylar</a>
                        </div>
                    </div>
                </div>
            <?php endwhile; ?>
